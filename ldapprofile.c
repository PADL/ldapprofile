...
static int getBaseForNisDomain(LDAP *ld, const char *nisDomain, char **baseForProfile);
static int connectToServer(char *host, LDAP **ld);
static char *getStringValue(LDAP *ld, LDAPMessage *entry, const char *attribute);
static char **getStringValues(LDAP *ld, LDAPMessage *entry, const char *attribute);
static int emitDUAConfigProfile(LDAP *ld, const char *base, const char *profile, const char *host, FILE *fp);
static void emitConfigurationHeader(LDAP *ld, LDAPMessage *e, FILE *fp);
static void emitConfKey_BASE(LDAP *ld, LDAPMessage *e, const char *defaultBase, FILE *fp);
static void emitConfKey_HOST(LDAP *ld, LDAPMessage *e, const char *defaultHost, FILE *fp);
static void emitConfKey_XXX(LDAP *ld, LDAPMessage *e, const char *attribute, const char *key, ProfileSyntax syn, FILE *fp);
...
int main(int argc, char **argv)
{
	...
	if (nisDomain != NULL) {
		...
		if (rc != LDAP_SUCCESS) {
			fprintf(stderr, "%s: Could not locate search base for NIS domain \"%s\"\n", argv[0],  nisDomain);
			exit(rc);
		}
		...
		if (profileBase != NULL) {
			if (strcasecmp(profileBase, base) != 0) {
				...
				fprintf(stderr, "%s: NIS domain \"%s\" does not match base \"%s\"\n",
	...
	rc = emitDUAConfigProfile(ld, profileBase, profileName, ldapServer, stdout);
	if (rc != LDAP_SUCCESS) {
		if (profileName != NULL) {
			fprintf(stderr, "%s: Could not retrieve DUA configuration profile \"%s\" at \"%s\"\n", argv[0], profileName, profileBase);
		} else {
			fprintf(stderr, "%s: Could not retrieve DUA configuration profile at \"%s\"\n", argv[0], profileBase);
		}
		exit(rc);
	}
...
static void usage(int argc, char **argv)
{
	fprintf(stderr, "%s: Usage: ldapprofile [-h ldapServer] [-b profileBase] [-p profileName] [-d nisDomain]\n", argv[0]);
	exit(1);
}
...
static int emitDUAConfigProfile(LDAP *ld, const char *base, const char *profileName, const char *hostWithProfile, FILE *fp)
{
	int rc;
#if 1
	...
	if (profileName == NULL) {
		...
		snprintf(filter, sizeof(filter), "(&(objectclass=DUAConfigProfile)(cn=%s))", profileName);
	}
	...
	rc = ldap_search_s(ld, base, LDAP_SCOPE_SUBTREE, filter, profileAttrs, 0, &profileRes);
	...
	emitConfKey_XXX(ld, profileEntry, "defaultSearchScope", "SCOPE", ProfileSyntaxScope, fp);
	emitConfKey_XXX(ld, profileEntry, "searchTimeLimit", "TIMELIMIT", ProfileSyntaxInteger, fp);
	emitConfKey_XXX(ld, profileEntry, "bindTimeLimit", "BIND_TIMELIMIT", ProfileSyntaxInteger, fp);
	emitConfKey_XXX(ld, profileEntry, "followReferrals", "REFERRALS", ProfileSyntaxBoolean, fp);
...
static int getBaseForNisDomain(LDAP *ld, const char *nisDomain, char **baseForProfile)
{
	...
	rc = ldap_search_s(ld, "", LDAP_SCOPE_BASE, "(objectclass=*)", rootDSEAttrs, 0, &rootDSERes);
	if (rc != LDAP_SUCCESS) {
		ldap_perror(ld, "ldap_search_s");
		return rc;
	}
	...
	*baseForProfile = NULL;
	...
	for (p = contexts; *p != NULL; p++) {
		...
		if (isUserContext) {
			...
			snprintf(nisDomainFilter, sizeof(nisDomainFilter), "(nisDomain=%s)", nisDomain);

			if (debug) {
				fprintf(stderr, "DEBUG: subtree search of \"%s\" for \"%s\"\n", *p, nisDomainFilter);
			}

			rc = ldap_search_s(ld, *p, LDAP_SCOPE_SUBTREE, nisDomainFilter, baseAttrs, 0, &baseRes);
			if (rc != LDAP_SUCCESS && rc != LDAP_SIZELIMIT_EXCEEDED) {
				continue;
			}
...
static void emitConfKey_HOST(LDAP *ld, LDAPMessage *e, const char *defaultHost, FILE *fp)
{
	...
	if (defaultServerList != NULL) {
		fprintf(fp, "%s%s", (preferredServerList != NULL ? " " : ""), defaultServerList);
	}
...
static void emitConfKey_BASE(LDAP *ld, LDAPMessage *e, const char *baseOfProfile, FILE *fp)
{
		...
		switch (syn) {
			case ProfileSyntaxBoolean:
				if (strcasecmp(value, "TRUE") && strcasecmp(value, "FALSE")) {
					fprintf(stderr, "warning: malformed boolean profile value %s: %s\n", attribute, value);
				}
			case ProfileSyntaxInteger:
				for (c = value; *c != '\0'; c++) {
					if (!isdigit(*c)) {
						fprintf(stderr, "warning: malformed integer profile value %s: %s\n", attribute, value);
						break;
					}
				}
				break;
			case ProfileSyntaxScope:
				if (strcmp(value, "base") &&
					strcmp(value, "one") &&
					strcmp(value, "sub")) {
					fprintf(stderr, "warning: malformed scopeSyntax profile value %s: %s\n", attribute, value);
				}
				break;
...
static char *chaseReferral(LDAP *ld, const char *service, char *referral)
{
	...
	rc = ldap_search_s(ld, base, LDAP_SCOPE_BASE, "(objectclass=DUAConfigProfile)", searchDescAttrs, 0, &res);
	if (rc != LDAP_SUCCESS) {
		ldap_perror(ld, "ldap_search_s");
		return NULL;
	}
	...
	if (descriptors == NULL) {
		fprintf(stderr, "warning: profile \"%s\" has no descriptor for service \"%s\"\n",
			base, service);
		ldap_msgfree(res);
		return NULL;
	}
	...
	for (p = descriptors; *p != NULL; p++) {
		...
		if (referredBase == NULL) {
			fprintf(stderr, "warning: malformed serviceSearchSyntax value serviceSearchDescriptor: %s\n", *p);
			continue;
		}
		...
		*base = '\0';
		...
		if (isValidNSSService(service)) {
			...
			while ((strncmp(chasedBase, "ref:", 4) == 0) && ++depth < 4) {
				...
				if (chasedBase == NULL) {
					fprintf(stderr, "warning: could not follow serviceSearchDescriptor referral \"%s\"\n", oldChasedBase);
					break;
				}
				free(oldChasedBase);
			}
